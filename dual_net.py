...
def conv_block(tensor, specs):
    tensor = layers.convolution2d(tensor, num_outputs=specs['num_outputs'],
...
def build_mlp(board_placeholder,
    ...
    for specs in shared_layers:
        if specs['layer'] == 'conv':
            ...
            out = layers.fully_connected(out, num_outputs=specs['num_outputs'],
    ...
    for specs in policy_head:
        if specs['layer'] == 'conv':
            ...
            policy_out = layers.fully_connected(policy_out, num_outputs=specs['num_outputs'],
                                                      activation_fn=specs['activation_fn'])
    ...
    for specs in value_head:
        if specs['layer'] == 'conv':
            ...
            value_out = layers.fully_connected(value_out, num_outputs=specs['num_outputs'],
                                                      activation_fn=specs['activation_fn'])
...
class DualNet:
    ...
    def __init__(self, sess, action_rep='pos', learning_rate=0.01, regularization_mult=0.01):
        ...
        self.regularization_loss = layers.apply_regularization(layers.l2_regularizer(regularization_mult), weights_list=tf.trainable_variables())
    ...
    def predict(self, inp):
        policy, value = self.sess.run([self.policy_predict, self.value_predict], feed_dict={self.board_placeholder: inp})
        return policy, value
